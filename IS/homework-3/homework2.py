...
def get_board_most_expensive_position(board):
    ...
    for i in range(0, n):
        index, value = get_board_row_most_expensive_position(board, i)
        most_expensive_positions_by_rows.append((value, index, i)) # (value, col, row)

    print_chess_board(board)
    print('Most expensive positions by rows: (value, col, row)')
...
def make_move(queens_positions, board, old_position):
    ...
    new_position = get_board_row_cheapest_position(board, old_position[2])
    print(new_position)
    queens_positions[old_position[2]] = new_position[0]

    print_queens_board(queens_positions)

    old_position = (old_position[2], old_position[1])

    board = calculate_board_weight(queens_positions, board, new_position[1], old_position)
    return queens_positions, board
...
def find_solution(queens_positions, board):
    ...
    for i in range(0, 100):
        ...
        queens_positions, board = make_move(queens_positions, board, most_expensive_position)
