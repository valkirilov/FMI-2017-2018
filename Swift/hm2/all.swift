...
func depth(root: VisualComponent?) -> Int {
    if let hasRoot = root {
        return depthOf(hasRoot, depth: 0)
    }
    else {
        return 0
    }
...
func depthOf(_ root: VisualComponent?, depth: Int) -> Int {
    if let hasRoot = root {
        if let isVisualGroup = hasRoot as? VisualGroup {
            var depths: [Int] = []
            for child in isVisualGroup.children {
                let currentDepth: Int = depthOf(child, depth: depth + 1)
...
func count(root: VisualComponent?, type: VisualComponent) -> Int {
    if let hasRoot = root {
        return countVisualComponentsOf(type, root: hasRoot, depth: 0)
    }
    else {
        return 0
    }
...
func countVisualComponentsOf(_ countedType: VisualComponent, root: VisualComponent?, depth: Int) -> Int {
    if let hasRoot = root {
        var currentDepth = depth
        if type(of: hasRoot) == type(of: countedType) {
            return depth + 1
        }
        else if let isVisualGroup = hasRoot as? VisualGroup {
            for child in isVisualGroup.children {
                currentDepth = countVisualComponentsOf(countedType, root: child, depth: currentDepth)
            }
            return currentDepth
        }
    }
...
// // Task 2 & 3
// var circle2: Circle = Circle(x: 0, y: 0, r: 3)
// var circle3: Circle = Circle(x: 0, y: 0, r: 3)
// vGroup.add(child: circle2)
// vGroup.add(child: triangle)
// vGroup.add(child: circle3)
// vGroup.add(child: rectangle)

// var hGroup2: HGroup = HGroup()
// var circle4: Circle = Circle(x: 0, y: 0, r: 3)
// var triangle2: Triangle = Triangle(a: point1, b: point2, c: point3)
// hGroup2.add(child: circle4)
// hGroup2.add(child: triangle2)
// hGroup2.add(child: hGroup)
// vGroup.add(child: hGroup2)

// print("Depth: \(depth(root: vGroup))") // it should be 3
// print("Number of circles: \(count(root: vGroup, type: circle))") // it should be five
// print("Number of circles: \(count(root: vGroup, type: triangle))") // it should be two
// print("Number of circles: \(count(root: vGroup, type: rectangle))") // it should be one